...
public class ProductOrderService {
    ...
    public ResponseEntity<ProductOrder> addProductOrder(ProductOrder order) {
        try {
            ResponseEntity<Product> forEntity = restTemplate.getForEntity
//                ("http://localhost:9090/product/" + order.getPrdId(), Product.class);
        ("http://PRODUCT-APPLICATION/product/" + order.getPrdId(), Product.class);
            System.out.println(forEntity.getStatusCode() + " is status code");

            if (forEntity.getStatusCode().is2xxSuccessful()) {
                return new ResponseEntity<>(orderRepository.save(order), HttpStatus.FOUND);
            } else
